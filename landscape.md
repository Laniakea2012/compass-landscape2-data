...
| 一级类别 | 二级类别 | 说明 |
|:-------:|:-------:|:-------:|
| 框架 | RN框架（React Native) | 基于 React 的 JavaScript 代码文件、适配不同平台的组件库、用于构建移动应用界面及交互逻辑的模块，以及与原生功能交互对接的桥接代码等，涉及 iOS 和 Android 双平台应用开发相关资源。 |
| 框架 | Flutter框架（Flutter） | Dart 语言编写的业务逻辑代码、Flutter 自带的丰富 UI 组件库、渲染引擎相关配置与代码，还有用于实现跨平台一致体验的布局、样式及交互代码等，旨在打造高性能跨平台移动应用的各类代码及配置文件。 |
| 媒体 | UI | 界面布局文件、各类交互控件代码、样式表定义，涵盖不同分辨率适配的界面元素设置、主题相关配置以及用户操作反馈机制实现代码等，专注于呈现给用户的可视化界面构建相关内容。 |
| 媒体 | 动画 | 动画效果实现代码（如帧动画、补间动画等）、动画控制逻辑（启动、暂停、结束等）、与界面元素结合的动画绑定代码，以及运用动画库实现特定动态视觉效果的相关资源，用于赋予媒体内容动态特性。 |
| 媒体 | 音视频 | 音视频采集设备调用代码、编码与解码算法实现、播放控制逻辑代码、编辑功能代码（如剪辑、特效添加等）以及相关流媒体处理机制代码，围绕音视频全流程处理的各类代码及配置。 |
| 媒体 | 图片 | 图片加载与缓存机制代码、显示优化算法（如缩放、裁剪、画质调整等）、图片格式转换工具代码、特效处理（滤镜、变形等）相关代码，专注于图片资源处理利用的代码及配置集合。 |
| 算法 | 人工智能 | 机器学习（如分类、回归算法等）、深度学习（神经网络结构及训练代码等）模型构建代码，数据预处理及特征工程相关代码，模型评估与优化算法代码，用于实现智能预测、识别等任务的整套代码及相关数据资源。 |
| 算法 | 数学及算法 | 数值计算方法（如迭代求解、插值算法等）代码、数据处理算法（排序、搜索等）、优化算法（线性规划、遗传算法等）实现代码，基于数学理论构建的各类通用或特定功能算法代码及相关辅助函数代码。 |
| 算法 | 图形 | 图形建模相关数据结构（顶点、边、面等定义）代码、渲染算法（光线追踪、光栅化等）代码、图形变换（平移、旋转、缩放等）逻辑代码、图形分析（碰撞检测、空间划分等）代码，围绕图形操作处理的各类代码及数据资源。 |
| 算法 | 加解密 | 对称加密（如 AES 等）、非对称加密（如 RSA 等）算法实现代码，哈希函数（如 MD5、SHA 等）代码，密钥生成与管理代码，加密通信协议实现代码，保障数据安全的整套加解密相关代码及配置。 |
| 应用开发 | 工具 | 代码编辑器插件、版本控制工具（如 Git 相关配置及脚本）、自动化构建工具（如 Gradle、Maven 等配置文件）、测试框架（单元测试、集成测试等代码及配置）、部署工具（容器化配置、发布脚本等），辅助开发全流程的各类工具及对应配置代码。 |
| 应用开发 | 架构及模式 | 应用分层架构设计代码（如表现层、业务逻辑层、数据访问层等）、设计模式（如 MVC、MVP、MVVM 等）实现代码，微服务架构相关配置（服务注册、发现等）、分布式架构通信及协调代码，用于构建合理应用架构的各类代码及配置资源。 |
| 应用开发 | 日志及调试 | 日志记录框架配置及写入代码（按不同级别记录运行信息等）、调试工具集成代码（如调试断点设置、变量查看等功能代码）、错误追踪与分析代码（堆栈信息处理、异常捕获等），保障应用开发及运行过程中问题排查的相关代码及配置。 |
| 应用开发| 辅助实用 | 时间处理函数（日期格式化、时间戳转换等）、本地化资源文件（多语言文本、地区格式适配等）、应用配置管理代码（参数读取、保存等）、辅助性功能（如缓存管理、通知推送等）相关代码，完善应用实用性的各类代码及配置资源。 |
| 数据 | 数据库 | 数据库管理系统（如 MySQL、Oracle 等）安装配置文件、数据库表结构定义（字段、类型、约束等）代码、数据存储过程及函数代码、索引创建与管理代码、数据库连接与操作（增删改查等）代码，围绕数据库使用及管理的整套代码及配置。 |
| 数据 | 文件及解析 | 不同文件类型（如 JSON、XML、CSV 等）读取与写入代码、文件格式解析算法代码、内容提取与转换逻辑代码、文件存储与检索机制代码，专注于文件资源处理利用的各类代码及配置资源。 |
| 网络协议 | 网络及协议 | 网络连接建立与关闭代码（如 TCP、UDP 连接等）、数据打包与解包逻辑（按协议格式封装、解析数据等）、网络通信协议（HTTP、HTTPS、WebSocket 等）实现代码、网络状态监测与错误处理代码，保障网络通信准确高效进行的相关代码及配置。 |
...
| 维度 | 指标 | 要求 | 与毕业指标要求对比 |
|:-------:|:-------:|:-------:|:-------:|
| 合法合规 | 许可证包含 | 项目本身许可证（含项目级和文件级）及其依赖软件许可证均为宽松类型许可证 | 增强 |
| 合法合规 | 版权声明 | 项目应明确版权归属，包括项目代码、文档等所有相关资源的版权信息清晰准确。在代码文件头部或文档开头等显著位置注明版权所有者、版权日期范围以及使用权限等相关内容 | 增强 |
| 合法合规 | 专利风险 | 对项目涉及的技术进行专利检索，确保项目的实现过程以及所包含的算法、架构等内容不存在侵犯他人专利的风险。如果项目使用了某些可能涉及专利的技术，需要提供相应的授权证明或者说明使用场景符合专利的合理使用范围 | 增强 |
| 合法合规 | 数据合规 | 若项目涉及数据收集、存储、处理和传输，必须遵守相关的数据保护法律法规。对于收集用户数据的开源项目，需要明确告知用户数据收集的目的、范围，并且获得用户的同意（如果适用）。数据存储过程应确保数据的安全性和保密性，数据传输过程要符合加密等安全要求 | 新增 |
| 网络安全 | 软件包签名 | 项目发布软件包包含数据签名并提供校验工具，保障下载包的完整可靠 | 增强 |
| 网络安全 | 安全漏洞 | 项目及依赖源码无公开已知未修复漏洞 | 增强 |
| 网络安全 | 漏洞响应 | 设立漏洞反馈渠道（如Issue 漏洞标签、漏洞反馈邮箱、论坛漏洞版块等），对用户的反馈进行及时回复和处理 | 增强 |
| 技术生态 | 代码规范 | 项目代码严格遵循统一的代码风格规范，如命名规则、缩进格式等，并且有自动化工具来检查和维护代码风格的一致性 | 新增 |
| 技术生态 | 测试覆盖率 | 项目建立完善的测试机制（测试框架和用例），单元测试覆盖率大于80%，并提供自动化检测工具/服务 | 增强 |
